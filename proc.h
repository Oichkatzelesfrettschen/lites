...
#if DUALCPU || PURDUE_EE
	struct	pte *p_pcbb;
	char	p_syscall;	/* syscall # being executed */
	char	p_run;		/* Processor this proc is running on */
	short	p_scalls;	/* like p_cpu, sched penalty for syscalls */
				/* p_scalls not implemented yet */
#endif
#if PURDUE_EE
#define	COMMSIZ	16		/* for IOP */
#define	TTYNSIZ  4
	char	p_comm[COMMSIZ];/* args to command -- not used yet */
	char	p_ttyn[TTYNSIZ];/* tty name for ps */
#endif
};
...
#if DUALCPU
#define	SNODUAL	0x2000000	/* Do not run this proc on slave cpu */
#define	SEXCSYS	0x4000000	/* doing excessive syscalls (not implemented) */
#endif
#if PURDUE_EE
#define	SCHD	0x8000000	/* Background cruncher job ("research" bit) */
#endif
